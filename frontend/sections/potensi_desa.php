...
<head>
    ...
    $potensi_per_halaman = 5;
    $halaman_sekarang = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    if ($halaman_sekarang < 1) $halaman_sekarang = 1;

    $offset = ($halaman_sekarang - 1) * $potensi_per_halaman;

    // Query total potensi desa
    $total_query = "SELECT COUNT(*) AS total FROM potensi_desa";
    $total_result = mysqli_query($db, $total_query);
    $total_data = mysqli_fetch_assoc($total_result)['total'];
    $total_halaman = ceil($total_data / $potensi_per_halaman);

    // Query potensi desa dengan limit & offset
    $query = "SELECT * FROM potensi_desa ORDER BY created_at DESC LIMIT $potensi_per_halaman OFFSET $offset";
...
<body>
    <div class="container">
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($p = mysqli_fetch_assoc($result)): ?>
                <div class="berita-item">
                    <a href="detail_potensi.php?id=<?= $p['id']; ?>" class="berita-judul">
                        <?= htmlspecialchars($p['judul']); ?>
                    </a>

                    <div class="berita-meta">
                        <?= htmlspecialchars($p['created_at']); ?> • Kategori: <?= htmlspecialchars($p['kategori']); ?>
                    </div>

                    <div class="berita-konten">
                        <img src="../../storages/potensi_desa/<?= htmlspecialchars($p['gambar']); ?>"
                            alt="<?= htmlspecialchars($p['judul']); ?>"
                            class="berita-gambar">

                        <div class="berita-teks">
                            <?= nl2br(mb_strimwidth(strip_tags($p['deskripsi']), 0, 300, '...')); ?>
                            <a href="detail_potensi.php?id=<?= $p['id']; ?>" class="selengkapnya">selengkapnya</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
        ...
        <?php else: ?>
            <p>Tidak ada data potensi desa tersedia.</p>
        <?php endif; ?>
