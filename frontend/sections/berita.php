...
$total_halaman = ceil($total_data / $berita_per_halaman);

// Query berita dengan limit & offset
$query = "SELECT * FROM berita ORDER BY created_at DESC LIMIT $berita_per_halaman OFFSET $offset";
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($b = mysqli_fetch_assoc($result)): ?>
                <div class="berita-item">
                    <a href="../sections/detail_berita.php?id=<?= $b['id']; ?>" class="berita-judul">
                        <?= htmlspecialchars($b['judul']); ?>
                    </a>

                    <div class="berita-meta">
                        <?= htmlspecialchars($b['tanggal']); ?> • <?= htmlspecialchars($b['penulis']); ?>
                    </div>

                    <div class="berita-konten">
                        <img src="../../storages/berita/<?= htmlspecialchars($b['gambar']); ?>"
                            alt="<?= htmlspecialchars($b['judul']); ?>"
                            class="berita-gambar">

                        <div class="berita-teks">
                            <?= nl2br(mb_strimwidth(strip_tags($b['isi']), 0, 300, '...')); ?>
                            <a href="../sections/detail_berita.php?id=<?= $b['id']; ?>" class="selengkapnya">selengkapnya</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
            ...
            <div class="pagination">
                ...
                <?php
                // Tampilkan nomor halaman (contoh sederhana: semua halaman)
