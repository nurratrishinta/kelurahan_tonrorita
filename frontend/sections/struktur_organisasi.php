...
<head>
    ...
    // Hitung total struktur organisasi
    $total_query = "SELECT COUNT(*) AS total FROM struktur_organisasi";
    $total_result = mysqli_query($db, $total_query);
    $total_data = mysqli_fetch_assoc($total_result)['total'];
    $total_halaman = ceil($total_data / $per_halaman);

    // Ambil data struktur organisasi
    $query = "SELECT * FROM struktur_organisasi ORDER BY parent_id ASC, id ASC LIMIT $per_halaman OFFSET $offset";
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($s = mysqli_fetch_assoc($result)): ?>
                <div class="berita-item">
                    ...
                    <a class="berita-judul">
                        <?= htmlspecialchars($s['nama']); ?>
                    </a>

                    <div class="berita-meta">
                        Jabatan: <?= htmlspecialchars($s['jabatan']); ?>
                        <?php if ($s['parent_id'] != 0): ?>
                            • Atasan ID: <?= htmlspecialchars($s['parent_id']); ?>
                        <?php endif; ?>
                    </div>

                    <div class="berita-konten">

                        <img src="../../storages/struktur_organisasi/<?= htmlspecialchars($s['foto']); ?>"
                            alt="<?= htmlspecialchars($s['nama']); ?>"
                            class="berita-gambar">

                        <div class="berita-teks">
                            Bagian dari struktur organisasi Kelurahan Tontorita.
                        </div>
