...
<head>
    ...
    // Hitung total data perangkat kelurahan
    $total_query = "SELECT COUNT(*) AS total FROM perangkat_kelurahan";
    $total_result = mysqli_query($db, $total_query);
    $total_data = mysqli_fetch_assoc($total_result)['total'];
    $total_halaman = ceil($total_data / $per_halaman);

    // Ambil data perangkat kelurahan
    $query = "SELECT * FROM perangkat_kelurahan ORDER BY created_at DESC LIMIT $per_halaman OFFSET $offset";
...
<body>
    <div class="container">
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($p = mysqli_fetch_assoc($result)): ?>
                <div class="berita-item">
                    ...
                    <a href="#" class="berita-judul">
                        <?= htmlspecialchars($p['nama']); ?>
                    </a>

                    <div class="berita-meta">
                        Jabatan: <?= htmlspecialchars($p['jabatan']); ?> • <?= htmlspecialchars($p['created_at']); ?>
                    </div>

                    <div class="berita-konten">

                        <img src="../../storages/perangkat_kelurahan/<?= htmlspecialchars($p['foto']); ?>"
                            alt="<?= htmlspecialchars($p['nama']); ?>"
                            class="berita-gambar">

                        <div class="berita-teks">
                            <?= nl2br(mb_strimwidth(strip_tags($p['keterangan']), 0, 300, '...')); ?>
                        </div>
        ...
        <?php else: ?>
            <p>Tidak ada data perangkat kelurahan tersedia.</p>
        <?php endif; ?>
