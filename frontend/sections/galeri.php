...
<head>
    ...
    $galeri_per_halaman = 5;
    $halaman_sekarang = isset($_GET['page']) ? (int)$_GET['page'] : 1;
    if ($halaman_sekarang < 1) $halaman_sekarang = 1;

    $offset = ($halaman_sekarang - 1) * $galeri_per_halaman;

    // Query total galeri
    $total_query = "SELECT COUNT(*) AS total FROM galeri";
    $total_result = mysqli_query($db, $total_query);
    $total_data = mysqli_fetch_assoc($total_result)['total'];
    $total_halaman = ceil($total_data / $galeri_per_halaman);

    // Query galeri dengan limit & offset
    $query = "SELECT * FROM galeri ORDER BY created_at DESC LIMIT $galeri_per_halaman OFFSET $offset";
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
            <?php while ($g = mysqli_fetch_assoc($result)): ?>
                <div class="berita-item">
                    <a href="../sections/detail_galeri.php?id=<?= $g['id']; ?>" class="berita-judul">
                        <?= htmlspecialchars($g['judul']); ?>
                    </a>

                    <div class="berita-meta">
                        <?= htmlspecialchars($g['created_at']); ?> • Kategori: <?= htmlspecialchars($g['kategori']); ?>
                    </div>

                    <div class="berita-konten">
                        <img src="../../storages/galeri/<?= htmlspecialchars($g['gambar']); ?>"
                            alt="<?= htmlspecialchars($g['judul']); ?>"
                            class="berita-gambar">

                        <div class="berita-teks">
                            <?= nl2br(mb_strimwidth(strip_tags($g['deskripsi']), 0, 300, '...')); ?>
                            <a href="../sections/detail_galeri.php?id=<?= $g['id']; ?>" class="selengkapnya">selengkapnya</a>
                        </div>
                    </div>
                </div>
            <?php endwhile; ?>
